...
export const createEmployeeSchema = z.object({
    ...
    // The onboarding form sends empty optional fields as `null`, so every
    // optional field must tolerate null (`.nullish()` = optional + nullable).
    // Previously these were `.optional()` (undefined-only) and rejected null,
    // which 400'd every create (e.g. "employeeId: expected string, received null").
    designationJustification: z.string().nullish(),
    taskTemplateLink: z.string().nullish(),
    department: z.string().nullish(),
    departmentId: z.string().nullish(),
    dateOfJoining: z.coerce.date().nullish(),
    dateOfBirth: z.coerce.date().nullish(),
    baseSalary: z.coerce.number().min(0).nullish(),
    ...
    employeeId: z.string().nullish(), // Custom ID like EMP-001
