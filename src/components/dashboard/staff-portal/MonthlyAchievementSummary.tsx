...
import { Calendar, ChevronLeft, ChevronRight, Pencil, Lock, TrendingUp, Award, IndianRupee, CheckSquare } from 'lucide-react';
...
export default function MonthlyAchievementSummary({ workReports }: MonthlyAchievementSummaryProps) {
    ...
    const { label, rows, totals, isCurrentMonth } = useMemo(() => {
        ...
        const rows = (workReports || [])
            ...
            .map((r) => ({
                id: r.id,
                dateStr: getISTDateString(new Date(r.date)),
                rawDate: r.date,
