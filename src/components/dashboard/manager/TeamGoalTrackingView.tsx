...
const TeamGoalTrackingView: React.FC = () => {
    ...
    useEffect(() => {
        const loadGoals = async () => {
            ...
            try {
                ...
                if (!res.ok) throw new Error(payload?.error || payload?.message || 'Failed to load goals');
    ...
    const completedGoals = goals.filter((g: any) => g.status === 'COMPLETED' || Number(g.currentValue || 0) >= Number(g.targetValue || 0)).length;

    let totalTargetSum = 0;
    let totalCurrentSum = 0;
    goals.forEach((g: any) => {
        const t = Number(g.targetValue || 0);
        const c = Number(g.currentValue || 0);
        if (t > 0) {
            totalTargetSum += t;
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div>
                ...
                <p className="text-secondary-500 font-medium">Monitoring goal progress against assigned KRA/KPI targets.</p>
            </div>

            {error && (
                <div className="card-premium p-4 text-sm font-semibold text-rose-600">
                    {error}
                </div>
            )}
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                        <div>
                            ...
                            <p className="text-indigo-100 text-sm mt-3 max-w-lg">
                                Based on {totalGoals} active goals tracked across the team. {completedGoals} goals have met or exceeded their target.
                            </p>
                        </div>
                        ...
                        <div className="w-full md:w-1/3 space-y-2">
                            ...
                            <div className="flex justify-end pt-1">
                                <span className="text-[10px] uppercase tracking-widest bg-white/20 px-2 py-0.5 rounded text-white font-bold backdrop-blur-sm">
                                    {teamProgressPercentage >= 80 ? 'On Track' : teamProgressPercentage >= 50 ? 'In Progress' : 'Needs Attention'}
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-6 relative">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-end">
                                    ...
                                    <p className="text-sm font-black text-secondary-900">{goal.currentValue || 0} / {goal.targetValue || 0} {goal.unit}</p>
                                </div>
                                <div className="w-full h-3 bg-secondary-100 rounded-full overflow-hidden p-0.5">
                                    <div
                                        className={`h-full rounded-full transition-all duration-1000 ${((goal.currentValue || 0) / Math.max(goal.targetValue || 1, 1)) >= 0.8 ? 'bg-emerald-500' :
                                                ((goal.currentValue || 0) / Math.max(goal.targetValue || 1, 1)) >= 0.5 ? 'bg-primary-500' :
                                                    'bg-amber-500'
                                            }`}
                                        style={{ width: `${Math.min(100, ((goal.currentValue || 0) / Math.max(goal.targetValue || 1, 1)) * 100)}%` }}
                                    ></div>
                                </div>
                            </div>

                            <div className="flex items-center justify-between pt-4 border-t border-secondary-50">
                                <span className={`px-2 py-0.5 text-[9px] font-black rounded uppercase tracking-[0.15em] ${goal.status === 'COMPLETED'
                                ...
                                <span className="text-[10px] font-bold text-secondary-400 uppercase">
                                    Due: {goal.endDate ? new Date(goal.endDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}
                                </span>
                            </div>
                        </div>
                    </div>
