...
const TeamPointsRewardsView: React.FC = () => {
    ...
    const topEarners = metrics?.topEarners || [];
    const totalLeaderboardPoints = topEarners.reduce((sum: number, row: any) => sum + Number(row.points || 0), 0);
    const avgLeaderboardPoints = topEarners.length > 0 ? totalLeaderboardPoints / topEarners.length : 0;
    const goalCompletion = Number(summary?.overallGoalAchievement || 0);
    const revenueAchievement = Number(summary?.overallAchievement || 0);
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex items-center gap-3 bg-white p-3 rounded-2xl shadow-xl shadow-secondary-100 border border-secondary-100">
                    ...
                    <div className="pr-4">
                        <p className="text-[10px] font-black text-secondary-400 uppercase tracking-widest leading-none">Leaderboard Participants</p>
                        <p className="text-lg font-black text-secondary-900 leading-none mt-1">{topEarners.length}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="space-y-8">
                        <div>
                            <div className="flex justify-between mb-3">
                                <span className="text-xs font-black uppercase tracking-widest text-secondary-400">Goal Completion</span>
                                <span className="text-xs font-black">{goalCompletion.toFixed(1)}%</span>
                            </div>
                            <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                                <div
                                    className="h-full bg-primary-500 rounded-full"
                                    style={{ width: `${Math.min(100, Math.max(0, goalCompletion))}%` }}
                                ></div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-6 pt-4">
                            <div className="p-6 bg-white/5 rounded-3xl border border-white/10 hover:bg-white/10 transition-colors cursor-pointer">
                                ...
                                <p className="text-2xl font-black">{revenueAchievement.toFixed(1)}%</p>
                                <p className="text-[10px] font-black text-secondary-400 uppercase tracking-widest">Revenue Achievement</p>
                            </div>
                            <div className="p-6 bg-white/5 rounded-3xl border border-white/10 hover:bg-white/10 transition-colors cursor-pointer">
                                <Award className="text-emerald-400 mb-3" size={24} />
                                <p className="text-2xl font-black">{avgLeaderboardPoints.toFixed(1)}</p>
                                <p className="text-[10px] font-black text-secondary-400 uppercase tracking-widest">Average Leaderboard Points</p>
                            </div>
                        </div>
                    </div>
                </div>
