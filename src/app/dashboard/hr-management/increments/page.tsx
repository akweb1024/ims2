...
export default function IncrementManagementPage() {
    ...
    const fetchIncrements = useCallback(async () => {
        try {
            ...
            const res = await fetch(`/api/hr/increments?${params}`, {
    ...
    const stats = {
        ...
        draft: increments.filter(i => i.status === 'DRAFT').length,
        pending: increments.filter(i => i.status === 'MANAGER_APPROVED').length,
        approved: increments.filter(i => i.status === 'APPROVED').length,
        rejected: increments.filter(i => i.status === 'REJECTED').length
    };
                ...
                <div className="card-premium overflow-hidden">
                    {loading ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-secondary-600 mt-4">Loading increments...</p>
                        </div>
