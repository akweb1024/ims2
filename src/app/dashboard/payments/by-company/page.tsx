 ...
 * to — a company tab per entry, reusing the existing /api/payments/razorpay endpoint (which
 * already enforces per-company access via assertCompanyAccess/canAccessAllCompanies), so a
 * regular employee naturally only ever sees their own company's single tab while a
 * SUPER_ADMIN sees one tab per company.
 */
export default function PaymentsByCompanyPage() {
    ...
    if (loadingCompanies) {
        return <div className="p-8 text-center text-secondary-500 font-bold">Loading companies…</div>;
    }
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div>
                <h1 className="text-3xl font-black text-secondary-900 tracking-tighter">Company Transactions</h1>
                <p className="text-secondary-500 font-medium italic">Razorpay payments, one company at a time.</p>
            </div>
