...
export const GET = authorizedRoute(
    ...
    async (req: NextRequest, user) => {
        try {
            ...
            if (!companyId) throw new ValidationError('Company context is required for this operation');
...
export const POST = authorizedRoute(
    ...
    async (req: NextRequest, user) => {
        try {
            ...
            // Fetch the count for projectCode generation
            ...
            const project = await prisma.iTProject.create({
                data: {
                    ...
                    projectManager: validatedData.projectManagerId ? { connect: { id: validatedData.projectManagerId } } : undefined,
                    teamLead: validatedData.teamLeadId ? { connect: { id: validatedData.teamLeadId } } : undefined,
                    department: validatedData.departmentId ? { connect: { id: validatedData.departmentId } } : undefined,
                    website: validatedData.websiteId ? { connect: { id: validatedData.websiteId } } : undefined,
                    ...
                    estimatedRevenue: !isNaN(parseFloat(validatedData.estimatedRevenue?.toString() || '0')) 
                        ? parseFloat(validatedData.estimatedRevenue!.toString()) : 0,
                    currency: validatedData.currency,
                    itDepartmentCut: !isNaN(parseFloat(validatedData.itDepartmentCut?.toString() || '0')) 
                    ...
                    taggedEmployees: validatedData.taggedEmployeeIds?.length ? {
                        connect: validatedData.taggedEmployeeIds.map((id: string) => ({ id }))
                    } : undefined,
            ...
            validatedData.taggedEmployeeIds?.forEach(id => { if (id !== user.id) recipients.add(id); });
            if (validatedData.projectManagerId && validatedData.projectManagerId !== user.id) recipients.add(validatedData.projectManagerId);
            if (validatedData.teamLeadId && validatedData.teamLeadId !== user.id) recipients.add(validatedData.teamLeadId);
