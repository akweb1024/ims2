      ...
      if (view === "all") {
        if (!ALL_ACCESS_ROLES.includes(user.role)) {
          throw new AuthorizationError(
            'Forbidden: Admin access required for "all" view',
          );
        }
      } else if (view === "team") {
        if (!IT_MANAGER_ROLES.includes(user.role)) {
          throw new AuthorizationError(
            'Forbidden: Manager access required for "team" view',
          );
        }
...
export const POST = authorizedRoute(
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      const effectiveItCut =
        validatedData.type === "SERVICE_REQUEST"
          ? 100
          : !isNaN(parseFloat(validatedData.itDepartmentCut?.toString() || "0"))
