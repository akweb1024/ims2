...
export async function GET(
  ...
  try {
        ...
        if (isManager) {
          ...
          if (!teamInvolved && task.project?.visibility !== "PUBLIC") {
            return NextResponse.json(
              { error: "Access Denied: Not in your team scope" },
              { status: 403 },
            );
          }
    ...
    const billableHours = task.timeEntries
    ...
    const taskWithStats = {
      ...
      stats: {
        ...
        estimatedVsActual: task.estimatedHours
          ? Math.round((totalTimeLogged / task.estimatedHours) * 100)
          : 0,
...
export async function PATCH(
  ...
  try {
        ...
        if (isPatchManager) {
          ...
          const teamInvolved =
            (existingTask.assignedToId &&
              teamIds.has(existingTask.assignedToId)) ||
            (existingTask.createdById &&
              teamIds.has(existingTask.createdById)) ||
            (existingTask.reporterId && teamIds.has(existingTask.reporterId));
          if (!teamInvolved) {
            return NextResponse.json(
              { error: "Forbidden: Not in your team scope" },
              { status: 403 },
            );
          }
        } else {
    ...
    if (
      updateData.actualValue !== undefined &&
      existingTask.itDepartmentCut > 0
    ) {
      updateData.itRevenueEarned =
        (updateData.actualValue * existingTask.itDepartmentCut) / 100;
    }
    ...
    if (
      ...
      (existingTask.type as string) === "SERVICE_REQUEST"
    ) {
      ...
      if ((existingTask.status as string) === "UNDER_REVIEW") {
        ...
        updateData.itRevenueEarned =
          ...
          existingTask.actualValue ||
          updateData.estimatedValue ||
          existingTask.estimatedValue;
...
export async function DELETE(
  ...
  try {
    ...
    const canDelete =
      ["SUPER_ADMIN", "ADMIN", "IT_MANAGER", "IT_ADMIN", "MANAGER"].includes(
        user.role,
      ) || task.createdById === user.id;
