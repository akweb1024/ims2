    ...
    try {
      ...
      if (
        ...
        (input.basePrice != null ||
          (input.basePrice === undefined && product.basePrice != null))
      ) {
      ...
      const updated = await db.$transaction(async (tx: any) => {
        const prod = await tx.invoiceProduct.update({
          ...
          data: {
            ...
            type: input.type,
            basePrice:
              input.basePrice !== undefined ? input.basePrice : undefined,
            category: input.category,
            pricingModel: input.pricingModel,
            description: input.description,
            shortDesc: input.shortDesc,
            priceINR: newType === "VARIABLE" ? 0 : input.priceINR,
            priceUSD: newType === "VARIABLE" ? 0 : input.priceUSD,
            priceTiers:
        ...
        if (input.variants !== undefined) {
          ...
          if (skus.length > 0) {
            ...
            if (existing)
              throw new ValidationError(
                `SKU "${existing.sku}" is already in use by another variant`,
              );
          }
