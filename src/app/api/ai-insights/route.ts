  ...
  try {
    ...
    if (type === "executive") {
      ...
      if (["MANAGER", "TEAM_LEADER"].includes(user.role)) {
        const subIds = await getDownlineUserIds(
          user.id,
      ...
      const aggData = (recentReports as any[]).reduce(
        (acc: WorkReportAgg, curr: any) => ({
      ...
      const productivityScore = recentReports.length
        ? (aggData.rating / recentReports.length).toFixed(1)
        : 0;
      const supportVelocity = recentReports.length
        ? (
            (aggData.tickets + aggData.chats + aggData.followups) /
    ...
    if (type === "hr") {
      ...
      const metrics = employees.map((emp: any) => {
        ...
        const avgCompliance = reports.length
          ? reports.reduce(
              (acc: number, curr: any) => acc + (curr.kraMatchRatio || 0),
              0,
            ) / reports.length
          : 0;
        const totalRevenue = reports.reduce(
          (acc: number, curr: any) => acc + (curr.revenueGenerated || 0),
          0,
        );

        return {
          name:
            (emp as any).user?.name || (emp as any).user?.email?.split("@")[0],
      ...
      // 2. High Value Alerts
      const lowCompliance = metrics.filter(
        (m) => m.reportCount > 3 && m.avgCompliance < 0.4,
      );
      const highContributors = metrics.filter(
        (m) =>
          m.totalRevenue > 20000 ||
          (m.avgCompliance > 0.8 && m.reportCount > 5),
      );

      const insights = [
        ...lowCompliance.map((m: any) => ({
      ...
      return NextResponse.json({
        metrics: {
          avgDailyProductivity: (
            metrics.reduce((acc: number, m: any) => acc + m.reportCount, 0) /
            (employees.length || 1)
          ).toFixed(1),
          flightRiskCount:
            employees.length -
            metrics.filter((m: any) => m.reportCount > 10).length,
    ...
    if (type === "productivity") {
      ...
      const analysis = (employees as any[])
        .map((emp) => {
          const reports = emp.workReports;
          const totals = reports.reduce(
            (acc: PerformanceMetrics, curr: any) => ({
          ...
          const avgRating =
            totals.ratingCount > 0 ? totals.rating / totals.ratingCount : 0;
          const avgKRA =
            reports.length > 0 ? totals.kraCompliance / reports.length : 0;
          const score =
            totals.tasks * 20 +
            totals.tickets * 25 +
            totals.revenue * 0.05 +
      ...
      const lowPerformers = analysis.filter(
        (emp: any) => emp.score < 50 || emp.avgRating < 2.5,
      );

      const summaries = topPerformers.map((emp) => {
        const reasons = [];
        if (emp.metrics.revenue > 10000)
          reasons.push(
            `generating ₹${emp.metrics.revenue.toLocaleString()} in revenue`,
          );
        if (emp.metrics.tasks > 20)
          reasons.push(`completing ${emp.metrics.tasks} high-priority tasks`);
        if (emp.metrics.tickets > 10)
          reasons.push(`resolving ${emp.metrics.tickets} support tickets`);
        if (emp.avgRating >= 4.5)
          reasons.push(
            `maintaining near-perfect quality ratings (★${emp.avgRating.toFixed(1)})`,
          );
      ...
      const warnings = lowPerformers.map((emp: any) => {
        const redFlags = [];
        if (emp.metrics.hours > 40 && emp.score < 20)
          redFlags.push("high activity but critically low output");
        if (emp.avgRating > 0 && emp.avgRating < 2)
          redFlags.push(
            `critically low manager satisfaction (★${emp.avgRating.toFixed(1)})`,
          );
        if (emp.metrics.hours === 0)
          redFlags.push("zero operational artifacts logged in this period");
    ...
    const currentAvg =
      revenueValues.length > 0
        ? revenueValues.reduce((a: number, b: number) => a + b, 0) /
    ...
    const churnRisks = expiringSubs
      .filter((sub: any) => {
        const recentPositiveComms = sub.customerProfile.communications.some(
          (c: any) =>
    ...
    const totalChurnRiskValue = churnRisks.reduce(
      (acc: number, c: any) => acc + c.value,
      0,
    );
    const totalUpsellValue = upsellOpportunities.reduce(
      (acc: number, c: any) => acc + c.value,
      0,
    );
