...
const calculatePotential = async (companyId: string) => {
  ...
  const avgMonthlyRevenue =
    stats.length > 0
      ? stats.reduce((acc: number, curr: any) => acc + curr.amount, 0) /
        stats.length
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      const shouldRecalculate =
        !potential ||
        new Date().getTime() - new Date(potential.lastCalculated).getTime() >
          1000 * 60 * 60;
    ...
    try {
      ...
      const potential = await prisma.companyPotential.upsert({
        ...
        create: {
          ...
          growthFactors: [
            {
              factor: "Global Digital Adoption",
              impact: "Accelerating",
              importance: 10,
            },
