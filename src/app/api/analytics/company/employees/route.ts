...
export const GET = authorizedRoute(
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      const processedEmployees = employees
        .map((emp: any) => {
          ...
          const totalTasks = reports.reduce(
            (sum: number, r: any) => sum + r.tasksCompleted,
            0,
          );
          const totalRevenue = reports.reduce(
            (sum: number, r: any) => sum + r.revenueGenerated,
            0,
          );
          const avgManagerRating =
            reports.length > 0
              ? reports.reduce(
                  (sum: number, r: any) => sum + (r.managerRating || 0),
                  0,
                ) / reports.length
              : 0;
          ...
          const lastIncrement = profile.lastIncrementDate
            ? new Date(profile.lastIncrementDate)
            : joiningDate;

          const now = new Date();
          const monthsSinceLastIncrement =
            (now.getFullYear() - lastIncrement.getFullYear()) * 12 +
            (now.getMonth() - lastIncrement.getMonth());
          const tenureMonths =
      ...
      const finalEmployees = processedEmployees.map((emp: any) => {
        const profile = employees.find(
          (e: any) => e.id === emp.id,
        )?.employeeProfile;
        const count =
          attendanceCounts.find((a: any) => a.employeeId === profile?.id)
            ?._count.id || 0;
