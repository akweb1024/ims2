...
const handler = authorizedRoute(
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      months.forEach((m) => {
        // Revenue Aggregation
        const monthlyInvoices = rawInvoices.filter((inv: any) => {
          const d = new Date(inv.createdAt);
          return (
            d.getMonth() + 1 === m.monthIndex && d.getFullYear() === m.year
          );
        });
        m.revenue = monthlyInvoices.reduce(
          (sum: number, inv: any) => sum + inv.total,
          0,
        );

        // Expense Aggregation (Payroll)
        const monthlyPayroll = rawPayroll.filter(
          (p: any) => p.month === m.monthIndex && p.year === m.year,
        );
        m.expense = monthlyPayroll.reduce(
          (sum: number, p: any) => sum + (p.ctc || p.amountPaid),
          0,
        );
      ...
      const growthRate =
        months[10].revenue > 0
          ? ((months[11].revenue - months[10].revenue) / months[10].revenue) *
            100
          : 0;
      ...
      const avgMonthlyRevenue =
        last3Months.reduce((sum, m) => sum + m.revenue, 0) / 3;
