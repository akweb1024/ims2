...
export default function PublicInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchInvoice = async (query: string) => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/public/invoices/${id}?${query}`);
      if (!res.ok) {
  ...
  // Links/QR codes generated by the app carry a per-invoice token — unlock
  // automatically. Without one, fall back to the manual access-code prompt.
  useEffect(() => {
    const token = new URLSearchParams(window.location.search).get("token");
    if (token) {
      fetchInvoice(`token=${encodeURIComponent(token)}`).then((ok) => {
        if (!ok) setError("This invoice link is invalid or expired. Enter the access code instead.");
      });
    }
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    await fetchInvoice(`accessCode=${encodeURIComponent(accessCode.trim())}`);
  };
