import { calculateInvoiceTaxBreakdown } from '../src/lib/invoice-tax.js';
...
breakdown.lineItems.forEach(item => {
  console.log(`- ${item.description}: Rate=${item.taxRate}%, Tax=${item.taxAmount}, Label=${item.taxRuleLabel}`);
});
