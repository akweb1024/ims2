...
async function main() {
  ...
  const companyPrefixMap = new Map(
    updatedCompanies.map((c: any) => [c.id, c.employeeIdPrefix || "EMP"]),
  );
  ...
  for (const emp of employees) {
    ...
    if (targetCompanyId) {
      prefix = (companyPrefixMap.get(targetCompanyId) as string) || "EMP";
    }
    ...
    if (
      emp.initialLeaveBalance === null ||
      emp.initialLeaveBalance === undefined
    ) {
      updates.initialLeaveBalance = 0;
      // Also ensure current leave balance aligns if missing
      if (
        emp.currentLeaveBalance === null ||
        emp.currentLeaveBalance === undefined
      ) {
    ...
    if (Object.keys(updates).length > 0) {
      ...
      console.log(
        `✅ Updated Profile: ${emp.id} -> New ID: ${updates.employeeId || employeeId}`,
      );
    }
    ...
    if (targetCompanyId) {
      ...
      if (!existingMapping) {
        ...
        console.log(
          `✅ Created Designation Mapping for Employee ${id} in Company ${targetCompanyId}`,
        );
      }
    }
  }
  ...
  for (const emp of employees) {
    ...
    if (!existingLedger) {
      ...
      await prisma.employeeProfile.update({
        where: { id: emp.id },
        data: {
          currentLeaveBalance: (emp.currentLeaveBalance || 0) + 1.5,
          leaveBalance: (emp.currentLeaveBalance || 0) + 1.5,
        },
      });
    }
  }
